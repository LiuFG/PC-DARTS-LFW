...
def _get_pixels(per_pixel, rand_color, const_white,patch_size, dtype=torch.float32, device='cuda'):
    ...
    if per_pixel:
        ...
        return 254*torch.ones((patch_size[0], 1, 1), dtype=dtype, device=device)
    else:
...
class RandomErasing:
    ...
    def __init__(
            self,
            probability=0.5, sl=0.02, sh=1/3, min_aspect=0.9,
        ...
        self.const_white = False
        if mode == 'rand':
            self.rand_color = True  # per block random normal
        elif mode == 'pixel':
            self.per_pixel = True  # per pixel random normal
        elif mode == 'const_white':
            self.const_white = True  # per pixel random normal
        else:
            ...
            for attempt in range(10):
                ...
                if w < img_w and h < img_h:
                    ...
                    img[:, top:top + h, left:left + w] = _get_pixels(
                        self.per_pixel, self.rand_color, self.const_white, (chan, h, w),
